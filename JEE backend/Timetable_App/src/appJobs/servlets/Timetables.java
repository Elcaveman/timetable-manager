	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("tt_id") != null)
		{
			...
			if(session.getAttribute("id") == null)
			{
	    		...
	    		session.setAttribute("denied", "Access Denied! login as Admin");
			...
			else 
			{
				...
				if(session.getAttribute("type").equals("admin") && tt.getTimetableUserFk() == (long)session.getAttribute("id") )
				{
				...
				{
					session.setAttribute("denied", "Access Denied! You don't own this time table");
					this.getServletContext().getRequestDispatcher("/Timetable_App/").forward(request, response);
				}
